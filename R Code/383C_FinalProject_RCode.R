...
setwd("/Users/jennstarling/UTAustin/2016_Fall_SDS 383C_Statistical Modeling 1/Final Project/MY_FILES/Data")

source(file="/Users/jennstarling/UTAustin/2016_Fall_SDS 383C_Statistical Modeling 1/Final Project/MY_FILES/R Code/Penalized_Matrix_Decomp_Functions.R")	#Read in Penalized Matrix Decomp functions.

#------------------------------------------------------------
#DATA LOADING:

#Read in data.
data2014 = read.csv(file="./2014FA_AllData.csv",header=T)
data2015 = read.csv(file="./2015FA_AllData.csv",header=T)
...
test.err.missing = sum(test.filled[,29] != yhat.temp) / length(yhat.filled)
test.err.missing

nobs.in.test = nrow(test.missing)
nobs.in.pred = sum(table.missing)

paste(nobs.in.test-nobs.in.pred,' values predicted as NA due to missing data.')
